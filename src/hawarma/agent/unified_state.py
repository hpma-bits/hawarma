"""
UnifiedState: 环境对 Agent/Strategy 暴露的统一观测

frozen=True 确保 Strategy 不会意外修改状态。
Strategy 的唯一输入，GameEnv 的 get_unified_state() 返回值。

输入: GameEnv 内部状态
输出: 不可变的 UnifiedState 快照
"""
...
class UnifiedState:
    ...
    """当前局可用配方 slug -> RecipeAdapter 的映射"""
