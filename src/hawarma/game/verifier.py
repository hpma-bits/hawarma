...
class Verifier:
    ...
    def _load_template(self) -> None:
        ...
        logger.info(f"Verifier loaded template from {empty_path} (threshold={threshold})")
    ...
    def is_assembly_empty(self) -> bool:
        ...
        if self._empty_template is None:
            logger.warning("Verifier: template not loaded, assuming empty")
            return True

        screen = G.DEVICE.snapshot()
        if screen is None:
            logger.warning("Verifier: failed to take snapshot")
            return True
