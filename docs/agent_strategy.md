...
| 优化 | 说明 | 状态 |
|------|------|------|
| 动态swipe参数 | 根据距离调整送餐参数（<400: 0.25s/12步，400-600: 0.3s/15步...） | ✅ 已实现 |
| 前瞻性烹饪 | 基于订单紧迫度提前烹饪，而非被动等待 | ✅ 已实现 |
| Serve失败重试 | 首次失败后重新扫描匹配订单（最早超时优先），无匹配则丢弃 | ✅ 已实现 |
| 食材超时防护 | WARN_THRESHOLD=4s，提前存入库存，优先增加同槽位数量 | ✅ 已实现 |
