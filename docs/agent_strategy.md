# Agent 核心算法与策略文档

## 1. 概述

本文档描述 Hawarma 烹饪游戏 Agent 的核心算法设计、策略分析和优化经验。

### 1.1 问题定义

- **目标**：在 90 秒内最大化完成订单数和得分
- **约束**：
  - 最多 4 个灶台同时烹饪
  - 食材烹饪完成后 5 秒内必须移走，否则过期
  - 订单有超时时间（普通 70 秒，Rush 40 秒）
  - 调料必须在食材齐全后才能添加

### 1.2 关键时间点

| 时间 | 事件 |
|------|------|
| t=0 | 游戏开始，无订单 |
| t=4 | 第一个订单出现（有1秒动画） |
| t=8 | 第二个订单出现 |
| 每4秒 | 新订单出现（如果有空槽位） |
| t=90 | 游戏结束 |

## 2. 数学模型

### 2.1 订单容量分析

```
订单到达率: 0.25个/秒 (1个/4秒)
理论最大订单数: 18个（基于平均处理时间5秒）
实际完成订单数: 13.4个（20局平均）
效率: 74%
```

### 2.2 单订单处理时间

| 类型 | 食材数 | 并行处理时间 | 串行处理时间 |
|------|--------|-------------|-------------|
| 简单订单 | 1 | 3-4秒 | 3-4秒 |
| 复杂订单 | 2 | 3-5秒 | 5-9秒 |

### 2.3 最新基准测试结果（20局平均）

| 指标 | 值 |
|------|-----|
| 完成订单 | 13.4个 |
| 超时订单 | 0.3个 |
| 超时率 | 2.2% |
| 平均得分 | 2376分 |
| 效率 | 74% |

## 3. 策略设计

### 3.1 阶段驱动决策模型

```
Assembly 阶段          允许的操作
─────────────────────────────────────────
NOT_READY           →  移动食材/取用库存/烹饪/清理/存储
NEEDS_SEASONING     →  添加调料
READY               →  送餐
```

### 3.2 优先级顺序

```
1. 送餐（最高优先级 - 释放assembly）
2. 移动完成食材（释放灶台）
3. 开始烹饪（让灶台尽早开始工作！）
4. 添加调料（可以在烹饪期间进行）
5. 清理过期食材
```

### 3.3 关键设计原则

1. **按需响应**：只烹饪当前订单需要的食材
2. **烹饪优先**：灶台是异步的，尽早开始烹饪可以让灶台更早工作
3. **避免预烹饪**：预烹饪的食材可能与订单不匹配

## 4. 实验结果

### 4.1 策略对比（30局平均）

| 策略 | 核心思想 | 完成订单 | 超时率 | 效率 |
|------|----------|----------|--------|------|
| naive | 调味优先 | 12.7个 | 4% | 71% |
| **cooking_first** | **烹饪优先** | **14.1个** | **2.3%** | **78%** |

### 4.2 验证结果

**有效优化**：
- ✓ 烹饪优先于调味（+10.8%）
- ✓ 优先烹饪时间长的食材（+2%）

**无效优化**：
- ✗ 预烹饪到assembly（与订单不匹配，-30%）
- ✗ 预烹饪到stockpile（额外开销，-2%）

### 4.3 烹饪优先的原理

**真实游戏的并行性**：
- swipe操作是串行的（每个0.1秒）
- 但灶台烹饪是异步的（开始后自动工作）

**烹饪优先的优势**：
```
swipe 1: 开始烹饪 → 灶台开始后台工作
swipe 2: 调味1 → 灶台在工作
swipe 3: 调味2 → 灶台在工作
swipe 4: 送餐 → 灶台在工作
```

**调味优先的劣势**：
```
swipe 1: 调味1 → 灶台空闲
swipe 2: 调味2 → 灶台空闲
swipe 3: 送餐 → 灶台空闲
swipe 4: 开始烹饪 → 灶台开始工作
```

**结论**：烹饪优先让灶台更早开始工作（早0.3秒），直接影响订单处理速度。

## 5. 经验教训

### 5.1 简单策略往往更优

按需响应策略已经能达到78%的效率。复杂的优化反而可能引入问题。

### 5.2 预烹饪的价值有限

**原因**：
- 无法预测订单，预烹饪的食材可能不匹配
- 预烹饪占用灶台，影响后续烹饪
- 存取stockpile有额外开销

**教训**：在不确定的环境中，按需响应比预测更可靠。

### 5.3 优先级顺序很重要

**正确的优先级**：
```
送餐 → 移动食材 → 开始烹饪 → 添加调料
```

**烹饪优先的原理**：
- 灶台是异步的（开始后自动工作）
- 调味是同步的（需要占用swipe时间）
- 尽早开始烹饪可以让灶台更早工作

### 5.4 理解真实游戏的并行性

- swipe操作是串行的（每个0.1秒）
- 但灶台烹饪是异步的（开始后自动工作）
- 模拟器不能完全反映真实游戏的并行性

### 5.5 测试要严谨

- 单局测试不可靠，需要多局平均（建议30局以上）
- 不同recipes组合表现差异大
- 要考虑厨具是否被使用（只统计需要该厨具的游戏）

### 5.6 不要过度依赖模拟器

- 模拟器可能不能完全反映真实游戏的行为
- 重要结论需要在真实环境中验证
- 从文档出发思考问题，而不是从代码出发

## 6. 结论

### 6.1 当前最优策略

**Cooking First策略**（烹饪优先）：
- 完成订单：14.1个
- 超时率：2.3%
- 效率：78%

### 6.2 进一步优化空间

理论上最大订单数为18个，当前完成14.1个，还有约22%的提升空间。但进一步优化可能需要：
- 更深入理解游戏机制
- 分析具体哪些订单导致超时
- 针对性优化复杂订单的处理

### 6.3 核心代码

**文件**：`hawarma/agent/agent.py`

**关键方法**：
- `step()`: 阶段驱动决策
- `_get_assembly_stage()`: 判断assembly阶段
- `_try_start_cooking()`: 开始烹饪（优先级高于调味）

### 6.4 重要原则

> **从文档出发思考问题，代码要与文档保持一致**
> 
> 不要过度依赖模拟器，重要结论需要在真实环境中验证。

## 7. 运行基准测试

```bash
python scripts/benchmark_agent.py --seeds 20
```
