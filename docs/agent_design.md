...
| 游戏时长 | 90 秒 | |
| 订单生成间隔 | 4 秒 | |
| 理论最大订单数 | 22 | 第 4, 8, ..., 88 秒生成 |
| 每订单最高分 | 220 | Rush(150) + 时间奖励(50) + 调料(20) |
| 理论最高总分 | 4840 | 22 × 220 |
| 理论平均总分 | 4015 | 25% Rush，平均 182.5/单 |
...
| Agent 版本 | tick | 平均订单 | 最大订单 | 平均分数 | 最高分数 |
|-----------|------|---------|---------|---------|---------|
| v1 | 0.5s | 8.9 | 15 | 1537 | 2793 |
| v1 | 0.1s | 11.3 | 17 | 1909 | 3214 |
| v2 | 0.1s | 14.7 | 20 | 2555 | 3379 |
| v2 | 0.05s | 14.9 | 20 | 2594 | 3526 |
...
| tick | 决策频率 | 典型延迟 | 影响 |
|------|---------|---------|------|
| 1.0s | 1次/秒 | 0-1s | 严重延迟，错过最佳时机 |
| 0.5s | 2次/秒 | 0-0.5s | 中等延迟 |
| 0.1s | 10次/秒 | 0-0.1s | 轻微延迟 |
| 0.05s | 20次/秒 | 0-0.05s | 接近实时 |
...
1. **更积极的预烹饪**：保持灶台始终忙碌
2. **流水线作业**：组装站送餐后立即开始下一份
3. **智能库存**：根据即将到来的订单预存食材
4. **双食材并行**：同时启动两个灶台
