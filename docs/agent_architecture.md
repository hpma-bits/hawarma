...
| 决策频率 | 0.05秒决策，0.2-0.5秒自适应扫描 |
| 优化方向 | 并行最大化（同时使用4个灶台） |
| 性能目标 | 优化平均分，而非单纯追求订单数量 |
| Serve失败处理 | 首次失败后重试匹配订单（最早超时优先），无匹配则丢弃 |
| 食材超时防护 | WARN_THRESHOLD=4s，优先增加同槽位数量 |
...
1. 烹饪优先于调味（+10.8%）
2. 优先烹饪时间长的食材（+2%）
3. **动态swipe参数**：根据距离调整送餐参数，提升成功率
4. **前瞻性烹饪**：基于订单紧迫度提前烹饪，而非被动等待
5. **Serve失败重试**：首次失败后重新扫描匹配订单（最早超时优先）
6. **食材超时防护**：WARN_THRESHOLD=4s，提前存入库存
