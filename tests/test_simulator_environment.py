...
class TestSimulatorEnvironment(unittest.TestCase):
    ...
    def test_assembly_property(self):
        ...
        self.assertIsInstance(assembly.ingredients_cookers, list)
        self.assertEqual(len(assembly.ingredients_cookers), 0)
    ...
    def test_start_cooking(self):
        ...
        if cookers:
            ...
            for order in self.env.orders:
                if order and hasattr(
                    self.sim.recipes[order.recipe_slug], "raw_ingredients"
                ):
