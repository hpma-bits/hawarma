...
class CookingAgent:
    ...
    def __init__(self, env, recipes: list):
        ...
        logger.info(
            f"CookingAgent ready | {len(recipes)} recipes | {len(self._ingredient_info_dict)} ingredients"
        )
    ...
    def _get_cooker_for_ingredient(
        self, ingredient: str, recipe: Recipe | None = None
    ...
    def _try_parallel_cooking(self) -> CookAction | None:
        ...
        # 组装站缺失的食材排最前
        needed_current = assembly_missing + [
            item for item in needed_current if item not in assembly_missing
        ]
        ...
        for slot in self.env.stockpile.values():
            if slot.count > 0 and slot.ingredient_name:
                stockpile_counts[slot.ingredient_name] = (
                    stockpile_counts.get(slot.ingredient_name, 0) + slot.count
                )
        ...
        for ing_name, cooker_type in needed_current:
            ...
            recipe = (
                self._recipe_by_slug.get(order_id)
                if (order_id := self._get_order_id_for_ingredient(ing_name))
                else None
            )
    ...
    def _try_start_cooking(self) -> CookAction | None:
    ...
    def _check_and_clear_expired_assembly(self) -> ClearAssemblyAction | None:
        ...
        for order in self.env.orders:
            if order and not order.done:
                ...
                if recipe:
                    recipe_ings = set(
                        self._get_recipe_attr(recipe, "raw_ingredients", [])
                    )
    ...
    def _check_stale_assembly(self) -> ClearAssemblyAction | None:
        ...
        assembly = self.env.assembly
        if assembly.is_free or not assembly.ingredients_cookers:
        ...
        ingredients_complete = self._ingredients_match(
            assembly.ingredients_cookers, recipe
        )
        condiments_needed = self._recipe_condiments.get(target_slug, {})
        condiments_complete = self._condiments_complete(
            assembly.condiments, condiments_needed
        )
    ...
    def _get_needed_ingredients(self) -> list[tuple[str, str]]:
        ...
        if target_slug:
            ...
            if recipe:
                ...
                if hasattr(recipe, "ingredients") and hasattr(
                    recipe.ingredients, "__iter__"
                ):
                    result = []
    ...
    def _can_add_to_assembly(
        self, ingredient: str, cooker_type: str | None = None
    ) -> bool:
    ...
    def _find_available_slot(self, ingredient: str, cooker_type: str) -> str | None:
        """找到可用的库存槽位"""
        for slot_name, slot in self.env.stockpile.items():
            if slot.ingredient_name is None or (
                slot.ingredient_name == ingredient and slot.cooker_type == cooker_type
            ):
                if slot.count < 5:
                    return slot_name
        return None

    def _try_increment_stockpile(self, ingredient: str, cooker_type: str) -> str | None:
        """增加现有库存槽位的数量（不移动到新槽位）"""
        for slot_name, slot in self.env.stockpile.items():
    ...
    def _get_order_id_for_ingredient(self, ingredient: str) -> int | None:
        """获取需要该食材的订单ID（优先级排序）"""
        for _, order in self._prioritized_orders():
            recipe = self._recipe_by_slug.get(order.recipe_slug)
            if recipe:
    ...
    def _ingredients_match(self, actual: list, recipe) -> bool:
        ...
        expected_raw = (
            ...
            else recipe.get("raw_ingredients", [])
        )
        expected_cookers = (
            recipe.cookers if hasattr(recipe, "cookers") else recipe.get("cookers", [])
        )
