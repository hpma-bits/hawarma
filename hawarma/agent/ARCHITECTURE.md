...
优先级 1: 送餐 (ServeOrderAction)
    ↓ (如果无法送餐)
优先级 2: 从灶台移到组装站 (MoveToAssemblyAction)
    ↓ (如果灶台没有完成的食材)
优先级 3: 开始烹饪 (CookAction)
    ↓ (如果所有灶台都在忙)
优先级 4: 添加调料 (AddCondimentAction)
    ↓ (如果无需调料)
优先级 5: 从库存取用到组装站 (PullFromStockpileAction)
    ↓ (如果库存没有需要的食材)
优先级 6: 清理过期食材 (ClearCookerAction)
    ↓ (如果没有过期食材)
优先级 7: 多余食材存入库存 (MoveToStockpileAction)
```

### 设计原理

1. **送餐优先**：完成订单是最终目标，一旦组装站有匹配订单的完整菜品立即送餐
2. **移动优先于烹饪**：已完成的食材应尽快移到组装站，释放灶台
3. **烹饪优先于调味**：灶台是异步的，尽早开始烹饪可以让灶台更早工作；调味可以在烹饪期间进行
4. **库存优先于新烹饪**：如果库存有需要的食材，直接取用比重新烹饪更快
5. **清理兜底**：过期食材占用灶台资源，必须清理
6. **库存缓冲**：多余的完成食材存入库存，以备后续订单使用
