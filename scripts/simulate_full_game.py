...
def run_full_simulation(seed: int = 42, recipe_file: str = "data/recipes.json"):
    sim = GameSimulator()
    ...
    while not env.is_game_over():
        ...
        for event in events:
            ...
            if et == "ORDER_APPEARED":
                ...
                print(f"[t={sim.time:.1f}s] NEW ORDER: {event.details['recipe']} ({rush_tag})")
            elif et == "ORDER_TIMEOUT":
                orders_timeout += 1
                print(f"[t={sim.time:.1f}s] ORDER TIMEOUT: {event.details.get('order_id', '?')}")
            elif et == "ORDER_SERVED":
                orders_served += 1
                score = event.details.get('score', 0)
                total_score += score
                print(f"[t={sim.time:.1f}s] SERVED order {event.details.get('order_id', '?')} | score={score}")
...
def _execute_and_log(env: SimulatorEnvironment, action: Action, game_time: float, sim) -> dict:
    ...
    if isinstance(action, CookAction):
        ...
        if ok:
            print(f"[t={game_time:.1f}s] COOK {action.ingredient} on {action.cooker} ({action.duration}s)")
        ...
        if ok:
            print(f"[t={game_time:.1f}s] PULL {action.slot} ({action.ingredient}) -> assembly")
        ...
        if result.success:
            ...
            for event in result.events:
                if event.event_type.name == "ORDER_SERVED":
                    ...
                    score = event.details.get('score', 0)
                    print(f"[t={game_time:.1f}s] SERVED order {order_id} | score={score}")
...
if __name__ == "__main__":
    ...
    run_full_simulation(seed=args.seed, recipe_file=args.recipes)
